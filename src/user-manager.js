...
module.exports.edit = (id, keypair) => {
  let json = JSON.parse(fs.readFileSync(dataManager.build('users', `${id}.json`)))
...
module.exports.getAttribute = (id, key) => {
  const user = module.exports.get(id)
...
module.exports.getKey = async (id) => {
  const user = await db.get('users', 'snowflake', id)
...
module.exports.getID = async (key) => {
  const user = await db.get('users', 'key', key)
...
module.exports.create = async (user, randomKey) => {
  if (fs.existsSync(dataManager.build('users', `${user.id}.json`))) { return }
  ...
  if (!(await module.exports.getKey(user.id))) { db.insert('users', ['snowflake', 'key'], [user.id, randomKey]) }
}
